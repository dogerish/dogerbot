// to import: Message = require(<filename>)(excd, config, echo, exists, sametype, Errors)
module.exports = function (excd, config, echo, exists, sametype, Errors)
{
	return class
	{
		constructor(msg, cmdman, context)
		{
			this.msg = msg;
			this.author = this.msg.author;
			this.guild = this.msg.guild;
			// filter out all the blank arguments
			this.splitup = this.msg.content.split(' ').filter(echo);
			...
			function argpush(i) { if (exists(me.allargs[i])) { me.args.push(me.allargs[i]); } }
			var i = 0;
		...
		checktype(inst, Obj, context) { if (!sametype(inst, Obj)) { return this.error.asreply('INVALID_TYPE', context); } else { return excd.cs[0]; } }
