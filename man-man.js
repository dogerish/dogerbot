// to import: ManualManager = require(<filename>)(fs, excd, exists, sametype, typecheck, listout, Exitcode, Message);
module.exports = function (fs, excd, exists, sametype, typecheck, listout, Exitcode, Message)
{
	return class
	{
		constructor(botcmds, msg) {
			// const err = typecheck(BotCommandManager, botcmds);
			// this.excd = (excd.vf(err)) ? excd.cs[0] : err;
			this.botcmds = botcmds;
			this.msg = (sametype(msg, Message)) ? msg : new Message(msg, this.botcmds);
		} // end constructor
		...
		fetch(alias, context, hideOptions, showArgs)
		{
			return this.readManArg(alias, function(manobj, buf, args, botcmd)
			{
				...
				if (showArgs && exists(args))
				 	manual = manual.replace("{args}", args.toString());
				else manual = manual.replace("{args}\n", '');

				return manobj.msg.reply(`Command Aliases: ${listout(botcmd.aliases)}\n${manual}`, context);
			});
		}
